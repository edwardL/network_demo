...
if(sp)
{
waypoints.push_back(JPS::Pos(sp - data[y], y));  /// 1-5 的点的x , y
}
}
}
...
{
bool found = findPath(path, grid, waypoints[i-1].x, waypoints[i-1].y , waypoints[i].x, waypoints[i].y, 
step, 0, &stepsDone , &nodesExpanded);
if(found)
{
...
{
std::cout<<"path not fount "<< std::endl;
}
}
